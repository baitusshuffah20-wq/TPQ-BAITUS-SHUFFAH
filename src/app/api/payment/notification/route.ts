...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `Payment status update: ${order_id} -> ${transaction_status} (${internalStatus})`,
    );
...
🌐 www.tpq-baitus-shuffah.com
      ...
      console.error(
        `Error sending WhatsApp confirmation for ${orderId}:`,
        error,
      );
    }
    ...
    try {
      ...
      console.error(
        `Error creating in-app notification for ${orderId}:`,
        error,
      );
    }
  } catch (error) {
...
async function sendPaymentFailureNotification(
  ...
  try {
    ...
    console.error(
      `Error sending payment failure notification for ${orderId}:`,
      error,
    );
  }
}
...
function getPaymentMethodText(paymentType: string): string {
  switch (paymentType?.toLowerCase()) {
    ...
    default:
      return paymentType || "Metode Pembayaran Digital";
  }
}
