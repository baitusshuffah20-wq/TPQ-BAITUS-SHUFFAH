...
export async function GET(request: NextRequest) {
  try {
    ...
    const config = {
      whatsapp: {
        accessToken:
          getSettingValue(settings, "whatsapp", "access_token") || "",
        phoneNumberId:
          getSettingValue(settings, "whatsapp", "phone_number_id") || "",
        businessAccountId:
          getSettingValue(settings, "whatsapp", "business_account_id") || "",
        webhookVerifyToken:
          getSettingValue(settings, "whatsapp", "webhook_verify_token") || "",
        appSecret: getSettingValue(settings, "whatsapp", "app_secret") || "",
        apiVersion:
          getSettingValue(settings, "whatsapp", "api_version") || "v18.0",
        isConfigured: Boolean(
          getSettingValue(settings, "whatsapp", "access_token"),
        ),
        status: getSettingValue(settings, "whatsapp", "access_token")
          ? "connected"
          : "disconnected",
      },
      email: {
