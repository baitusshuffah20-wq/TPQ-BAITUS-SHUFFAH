  ...
  try {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          "Could not determine the correct table name for hafalan progress",
        );
      }
    }
  }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log("GET /api/hafalan-progress - Query params:", {
      santriId,
      status,
      surahId,
      search,
  ...
  try {
    ...
    if ((existingRows as any[]).length > 0) {
      return NextResponse.json(
        {
          success: false,
          message: "Progress hafalan untuk surah ini sudah ada",
        },
        { status: 400 },
      );
    }
    ...
    const halaqah =
      (halaqahRows as any[]).length > 0 ? (halaqahRows as any[])[0] : null;
