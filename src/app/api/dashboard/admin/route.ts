...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      // Total hafidz (santri with at least 30 juz)
      prisma.santri.count({
        where: {
          status: 'ACTIVE',
          hafalan: {
    ...
    const donationsChange = donationsPrevMonthAmount > 0 
      ? Math.round(((donationsThisMonthAmount - donationsPrevMonthAmount) / donationsPrevMonthAmount) * 100) 
      : 0;
    
    const sppThisMonthAmount = sppThisMonth._sum.paidAmount || 0;
    const sppPrevMonthAmount = sppPrevMonth._sum.paidAmount || 0;
    const sppChange = sppPrevMonthAmount > 0 
      ? Math.round(((sppThisMonthAmount - sppPrevMonthAmount) / sppPrevMonthAmount) * 100) 
      : 0;
    ...
    const formattedEvents = upcomingEvents.map(event => {
      const eventDate = event.publishedAt || new Date();
      return {
        id: event.id,
        title: event.title,
        date: eventDate,
