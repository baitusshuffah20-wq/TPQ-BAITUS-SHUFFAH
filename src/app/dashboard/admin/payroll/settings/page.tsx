...
export default function PayrollSettingsPage() {
  ...
  const [editingSetting, setEditingSetting] = useState<PayrollSetting | null>(
    null,
  );
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [settingToDelete, setSettingToDelete] = useState<PayrollSetting | null>(
    null,
  );
    ...
    if (settingToDelete) {
      try {
        setPayrollSettings((prev) =>
          prev.filter((s) => s.id !== settingToDelete.id),
        );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingSetting) {
        ...
        setPayrollSettings((prev) =>
          prev.map((s) => (s.id === editingSetting.id ? updatedSetting : s)),
        );
            ...
            >
              <CardContent className="p-6">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        <p className="text-xs font-medium text-gray-600">
                          Honor per Jadwal Halaqah
                        </p>
                        <p className="text-base font-medium text-gray-900">
                          {formatCurrency(setting.baseAmount)}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs font-medium text-gray-600">
                        ...
                        <p className="text-base font-medium text-green-600">
                          {formatCurrency(setting.bonusAmount)}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <p className="text-base font-medium text-red-600">
                          {formatCurrency(setting.lateDeduction)}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs font-medium text-gray-600">
                        ...
                        <p className="text-base font-medium text-red-600">
                          {formatCurrency(setting.absentDeduction)}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center text-xs text-gray-500">
                      <span>
                        Terakhir diperbarui: {formatDate(setting.updatedAt)}
                      </span>
                    </div>
                  </div>
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 flex justify-between items-center">
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${setting.isActive ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}
                  >
                    {setting.isActive ? (
                      <>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      toast.success(
                        `Pengaturan ${setting.isActive ? "dinonaktifkan" : "diaktifkan"}`,
                      );
                    }}
                  >
        ...
        {filteredSettings.length === 0 && (
          <div className="text-center py-12 bg-white rounded-lg shadow">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              Tidak ada pengaturan penggajian
            </h3>
            <p className="text-gray-600 mb-6">
              {searchTerm
                ? "Tidak ada pengaturan yang sesuai dengan pencarian"
                : "Belum ada pengaturan penggajian yang ditambahkan"}
            </p>
            {searchTerm ? (
            ...
            <div className="bg-white rounded-lg shadow-lg w-full max-w-md">
              <div className="p-6">
                <h3 className="text-lg font-medium text-gray-900 mb-4">
                  {editingSetting
                    ? "Edit Pengaturan Penggajian"
                    : "Tambah Pengaturan Penggajian"}
                </h3>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSettingForm({
                            ...settingForm,
                            lateDeduction: parseInt(e.target.value),
                          })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                        required
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSettingForm({
                            ...settingForm,
                            absentDeduction: parseInt(e.target.value),
                          })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                        required
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                if (e.target.checked) {
                                  ...
                                  setSettingForm({
                                    ...settingForm,
                                    appliesTo: settingForm.appliesTo.filter(
                                      (p) => p !== position,
                                    ),
                                  });
                                }
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-lg w-full max-w-md">
              <div className="p-6">
                ...
                <p className="text-gray-600 mb-6">
                  Apakah Anda yakin ingin menghapus pengaturan penggajian "
                  {settingToDelete.name}"? Tindakan ini tidak dapat dibatalkan.
                </p>
