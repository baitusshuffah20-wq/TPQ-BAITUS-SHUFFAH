...
export default function PayrollReportsPage() {
  ...
  const loadReports = async () => {
    try {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        const present = Math.floor(totalSchedules * (0.8 + Math.random() * 0.15));
        ...
        reports.push({
          ...
          generatedAt: new Date(year, month - 1, endDate.getDate() - 2).toISOString(),
          finalizedAt: i === 0 ? undefined : new Date(year, month - 1, endDate.getDate()).toISOString(),
          musyrifBreakdown: [
            {
              musyrifId: 'musyrif_1',
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'FINALIZED': return <CheckCircle className="h-4 w-4 text-green-600" />;
            ...
            <Card>
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <p className="text-sm font-medium text-gray-600">Total Penggajian</p>
                    <p className="text-2xl font-bold text-gray-900">{formatCurrency(payrollReports[0].totalPayroll)}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <p className="text-sm font-medium text-gray-600">Total Musyrif</p>
                    <p className="text-2xl font-bold text-green-600">{payrollReports[0].totalMusyrifs}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <div className="flex items-center">
                      <p className="text-2xl font-bold text-purple-600">{payrollReports[0].attendanceRate}%</p>
                      {payrollReports.length > 1 && (
                        <span className="ml-2">
                          {getTrendIcon(payrollReports[0].attendanceRate, payrollReports[1].attendanceRate)}
                        </span>
                      )}
                    </div>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <div className="flex items-center">
                      <p className="text-2xl font-bold text-yellow-600">{payrollReports[0].punctualityRate}%</p>
                      {payrollReports.length > 1 && (
                        <span className="ml-2">
                          {getTrendIcon(payrollReports[0].punctualityRate, payrollReports[1].punctualityRate)}
                        </span>
                      )}
                    </div>
                  </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Gaji</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Musyrif</th>
                    ...
                    <tr key={report.id} className="hover:bg-gray-50">
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">
                          {getMonthName(report.period.month)} {report.period.year}
                        </div>
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{formatCurrency(report.totalPayroll)}</div>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{report.totalMusyrifs} musyrif</div>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{report.totalSchedules} jadwal</div>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          <div className="flex items-center">
                            <span className="text-green-600">{report.attendanceRate}%</span>
                            <span className="mx-1">•</span>
                            <span className="text-xs text-gray-600">
                              {report.totalAttendance.present} hadir, {report.totalAttendance.late} terlambat
                            </span>
                          </div>
                        </div>
                      </td>
              ...
              {payrollReports.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">Tidak ada laporan penggajian</h3>
                  <p className="text-gray-600">
                ...
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="text-lg font-medium text-gray-900">
                      Laporan Penggajian - {getMonthName(selectedReport.period.month)} {selectedReport.period.year}
                    </h3>
                    <p className="text-sm text-gray-600">
                      {formatDate(selectedReport.period.startDate)} - {formatDate(selectedReport.period.endDate)}
                    </p>
                  </div>
                  <div className="flex space-x-2">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(selectedReport.status)}`}>
                      ...
                      <span className="ml-1">
                        {selectedReport.status === 'DRAFT' ? 'Draft' : 'Finalisasi'}
                      </span>
                    </span>
                    <span className="mx-2 text-gray-400">•</span>
                    <span className="text-sm text-gray-600">
                      Dibuat pada {formatDate(selectedReport.generatedAt.split('T')[0])}
                    </span>
                    {selectedReport.finalizedAt && (
                      <>
                        <span className="mx-2 text-gray-400">•</span>
                        <span className="text-sm text-gray-600">
                          Difinalisasi pada {formatDate(selectedReport.finalizedAt.split('T')[0])}
                        </span>
                      </>
                    )}
                  </div>
                      ...
                      <CardContent className="p-4">
                        <div className="flex justify-between items-center">
                          <div>
                            ...
                            <p className="text-xl font-bold text-gray-900">{formatCurrency(selectedReport.totalPayroll)}</p>
                          </div>
                    ...
                    <Card>
                      <CardContent className="p-4">
                        <div className="flex justify-between items-center">
                          <div>
                            <p className="text-sm font-medium text-gray-600">Tingkat Kehadiran</p>
                            <p className="text-xl font-bold text-green-600">{selectedReport.attendanceRate}%</p>
                          </div>
                    ...
                    <Card>
                      <CardContent className="p-4">
                        <div className="flex justify-between items-center">
                          <div>
                            <p className="text-sm font-medium text-gray-600">Tingkat Ketepatan</p>
                            <p className="text-xl font-bold text-yellow-600">{selectedReport.punctualityRate}%</p>
                          </div>
                    ...
                    <CardContent className="pt-2">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                          ...
                          <p className="text-lg font-medium text-gray-900">{selectedReport.totalSchedules}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-gray-600">Hadir</p>
                          <p className="text-lg font-medium text-green-600">{selectedReport.totalAttendance.present}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-gray-600">Terlambat</p>
                          <p className="text-lg font-medium text-yellow-600">{selectedReport.totalAttendance.late}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-gray-600">Tidak Hadir</p>
                          <p className="text-lg font-medium text-red-600">{selectedReport.totalAttendance.absent}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent className="pt-2">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          ...
                          <p className="text-lg font-medium text-gray-900">{formatCurrency(selectedReport.totalBaseAmount)}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-gray-600">Potongan</p>
                          <p className="text-lg font-medium text-red-600">-{formatCurrency(selectedReport.totalDeductions)}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-gray-600">Bonus</p>
                          <p className="text-lg font-medium text-green-600">+{formatCurrency(selectedReport.totalBonuses)}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-base">Rincian per Musyrif</CardTitle>
                    </CardHeader>
                    <CardContent className="pt-2">
                      <table className="w-full border-collapse">
                        <thead>
                          <tr className="bg-gray-50 border-b border-gray-200">
                            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Musyrif</th>
                            ...
                            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Honor</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200">
                          {selectedReport.musyrifBreakdown.map(musyrif => (
                            <tr key={musyrif.musyrifId} className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                                <div className="flex items-center">
                                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                                    musyrif.attendanceRate >= 90 ? 'bg-green-100 text-green-800' :
                                    musyrif.attendanceRate >= 80 ? 'bg-blue-100 text-blue-800' :
                                    musyrif.attendanceRate >= 70 ? 'bg-yellow-100 text-yellow-800' :
