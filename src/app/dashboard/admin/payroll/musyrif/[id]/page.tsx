      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (dayOfWeek === 1 || dayOfWeek === 3 || dayOfWeek === 5) {
          ...
          mockAttendances.push({
            ...
            notes:
              status === "ABSENT" ? "Tidak hadir tanpa keterangan" : undefined,
          });
        }
      }
    ...
    try {
      if (payroll) {
        setPayroll({
          ...
          status: newStatus,
          paidAt:
            newStatus === "PAID" ? new Date().toISOString() : payroll.paidAt,
        });
        ...
        <div className="flex items-center justify-center min-h-96">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">
              Penggajian dengan ID {payrollId} tidak ditemukan
            </p>
            <Button
              onClick={() => router.push("/dashboard/admin/payroll/musyrif")}
            >
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600">
                {musyrif.name} - {getMonthName(payroll.period.month)}{" "}
                {payroll.period.year}
              </p>
            </div>
          </div>
          ...
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="flex items-center">
                <span
                  className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(payroll.status)}`}
                >
                ...
                <span className="text-sm text-gray-600">ID: {payroll.id}</span>
              </div>
              <div className="flex flex-col md:flex-row md:items-center gap-4">
                <div className="text-sm text-gray-600">
                  <span className="font-medium">Dibuat:</span>{" "}
                  {formatDate(payroll.generatedAt.split("T")[0])}
                </div>
                {payroll.paidAt && (
                  <>
                    ...
                    <div className="text-sm text-gray-600">
                      ...
                      {formatDate(payroll.paidAt.split("T")[0])}
                    </div>
                  </>
                )}
              </div>
              ...
              <div className="space-y-4">
                ...
                {musyrif.bankAccount && (
                  <div className="flex items-start">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">
                        {musyrif.bankAccount.bankName}
                      </p>
                      <p className="text-xs text-gray-600">
                        {musyrif.bankAccount.accountNumber}
                      </p>
                      <p className="text-xs text-gray-600">
                        {musyrif.bankAccount.accountHolder}
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-600">
                      {formatDate(payroll.period.startDate)} -{" "}
                      {formatDate(payroll.period.endDate)}
                    </p>
                  </div>
                  <div>
                ...
                <div className="border-t border-gray-200 pt-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600">
                        {payroll.lateSchedules > 0 &&
                          `${payroll.lateSchedules} terlambat × Rp 10.000`}
                        {payroll.lateSchedules > 0 &&
                          payroll.absentSchedules > 0 &&
                          ", "}
                        {payroll.absentSchedules > 0 &&
                          `${payroll.absentSchedules} absen × Rp 50.000`}
                      </p>
                    </div>
                  </div>
                  ...
                  {attendances.map((attendance) => (
                    <tr key={attendance.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(attendance.status)}`}
                        >
