...
interface ThemeContextType {
  ...
  isLoading: boolean;
  error: string | null;
  saveTheme: (theme: Partial<ThemeConfig> & { name: string }) => Promise<Theme | null>;
  updateTheme: (id: string, theme: Partial<ThemeConfig> & { name: string }) => Promise<Theme | null>;
...
export const defaultTheme: ThemeConfig = {
  ...
  buttons: {
    ...
    accent: '#00e0e0',     // Cyan terang
    danger: '#EF4444',     // Merah
    info: '#00bcd4',       // Biru muda, harmonis dengan accent
  },
  fonts: {
...
export const ThemeProvider = ({ children }: { children: ReactNode }) => {
  ...
  interface ThemeWithOptionalButtons extends Partial<Theme> {
  ...
  const ensureCompleteTheme = (themeData: Partial<ThemeConfig>): ThemeConfig => {
    ...
    return {
      colors: {
        ...
        background: themeData.colors?.background || defaultTheme.colors.background,
      ...
      buttons: {
        primary: themeData.buttons?.primary || themeData.colors?.primary || defaultTheme.buttons.primary,
        secondary: themeData.buttons?.secondary || themeData.colors?.secondary || defaultTheme.buttons.secondary,
        accent: themeData.buttons?.accent || themeData.colors?.accent || defaultTheme.buttons.accent,
        danger: themeData.buttons?.danger || themeData.colors?.error || defaultTheme.buttons.danger,
        info: themeData.buttons?.info || themeData.colors?.accent || defaultTheme.buttons.info,
      },
      fonts: {
      ...
      layout: {
        borderRadius: themeData.layout?.borderRadius || defaultTheme.layout.borderRadius,
        containerWidth: themeData.layout?.containerWidth || defaultTheme.layout.containerWidth,
        sidebarStyle: themeData.layout?.sidebarStyle || defaultTheme.layout.sidebarStyle,
      },
      logo: {
  ...
  const saveTheme = async (themeData: Partial<ThemeConfig> & { name: string }): Promise<Theme | null> => {
    try {
      ...
      if (!responseData.success || !responseData.theme) {
        const errorMessage = responseData.error || 'Failed to save theme: Invalid response data';
        toast.error(errorMessage);
        throw new Error(errorMessage);
      }
  ...
  const updateTheme = async (id: string, themeData: Partial<ThemeConfig> & { name: string }): Promise<Theme | null> => {
    try {
      ...
      if (authUserStr) {
        document.cookie = `auth_user=${encodeURIComponent(authUserStr)};path=/;max-age=86400`;
      }
