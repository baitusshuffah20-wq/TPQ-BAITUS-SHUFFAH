...
export async function getBadges(filters?: {
  ...
  const response = await fetch(`/api/achievements/badges?${params.toString()}`, {
    method: 'GET',
    headers: {
...
export async function getSantriAchievements(filters?: {
  ...
  const response = await fetch(`/api/achievements/santri?${params.toString()}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    },
...
export async function generateAchievementCertificate(id: string) {
  ...
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.error || 'Failed to generate achievement certificate');
  }
...
export async function downloadAchievementCertificate(id: string) {
  ...
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.error || 'Failed to download achievement certificate');
  }
