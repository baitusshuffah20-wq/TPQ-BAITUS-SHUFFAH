...
export async function createDonationPayment(paymentData: PaymentRequest) {
  try {
    ...
    if (
      ...
      !paymentData.customerDetails.firstName
    ) {
      return {
        success: false,
    ...
    try {
      if (!snap) {
        ...
        return {
          success: false, // Return false on transaction creation error
          token: "mock-token-" + Date.now(),
          redirectUrl: `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/donation/error?order_id=${paymentData.orderId}&dev_mode=true`, // Redirect to error page
          devMode: true,
        };
      }
      ...
      if (!transaction || !transaction.token) {
        ...
        return {
          success: false, // Return false on invalid transaction response
          token: "mock-token-" + Date.now(), // Still provide a mock token for consistency, but success is false
          redirectUrl: `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/donation/error?order_id=${paymentData.orderId}&dev_mode=true`, // Redirect to error page
          devMode: true,
      ...
      return {
        ...
        devMode: true,
        error:
          "Failed to create transaction with payment gateway, using development mode",
        details:
