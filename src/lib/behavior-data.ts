...
// Helper Functions
export const getBehaviorCategoryColor = (
  category: BehaviorCategory,
): string => {
  switch (category) {
    case "AKHLAQ":
...
export const getBehaviorSeverityColor = (
  severity: BehaviorSeverity,
): string => {
  switch (severity) {
    case "LOW":
...
export const getCharacterGrade = (
  score: number,
): BehaviorSummary["characterGrade"] => {
...
export const getGradeColor = (
  grade: BehaviorSummary["characterGrade"],
): string => {
  switch (grade) {
    case "A+":
    case "A":
...
// A minimal interface for records that can be used for trend calculation
export interface TrendableRecord {
  date: string | Date;
  points: number;
}

export const calculateTrend = (
  records: TrendableRecord[],
  days: number = 7,
): {
  improving: boolean;
  stable: boolean;
  ...
  const recentAverage =
    recentRecords.reduce((sum, r) => sum + r.points, 0) / recentRecords.length;
  const olderAverage =
    olderRecords.reduce((sum, r) => sum + r.points, 0) / olderRecords.length;
...
export const BEHAVIOR_CRITERIA: BehaviorCriteria[] = [
  ...
  },
  {
    ...
    islamicReference: {
      ...
      explanation:
        "Bukan termasuk golongan kami yang tidak menyayangi yang kecil dan tidak menghormati yang besar",
    },
    createdAt: "2024-01-01T00:00:00Z",
    updatedAt: "2024-01-01T00:00:00Z",
  },
  ...
  // IBADAH - Positive
  {
    ...
    rewards: [
      "Sertifikat rajin shalat",
      "Hadiah Al-Quran kecil",
      "Pujian khusus",
    ],
    islamicReference: {
  ...
  // DISCIPLINE - Negative
  {
    ...
    islamicReference: {
      ...
      explanation:
        "Waktu seperti pedang, jika kamu tidak memanfaatkannya maka ia akan merugikanmu",
    },
    createdAt: "2024-01-01T00:00:00Z",
    updatedAt: "2024-01-01T00:00:00Z",
  },
