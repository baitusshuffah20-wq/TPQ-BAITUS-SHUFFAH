...
class AIEngine {
  ...
  predictCompletionTime(
    ...
    const daysSinceStart = dates.map(
      (date) => (date - startDate) / (1000 * 60 * 60 * 24),
    );
    ...
    const daysToComplete =
      (remainingSurah / regression.slope) * adjustmentFactor;

    // Calculate completion date
    const completionDate = new Date();
    completionDate.setDate(
      completionDate.getDate() + Math.round(daysToComplete),
    );
    ...
    if (avgGrade < 80)
      suggestions.push("Focus on improving tajwid and pronunciation");
    if (attendanceRate < 0.9)
      suggestions.push("Improve attendance consistency");
    if (regression.slope > 0.2)
      suggestions.push("Maintain current excellent pace");
  ...
  generateRecommendations(student: StudentData): SmartRecommendation[] {
    ...
    if (avgRecentGrade < 75) {
      recommendations.push({
        ...
        description:
          "Recent grades indicate need for additional practice and review",
        priority: "HIGH",
        actionItems: [
    ...
    if (gradeVariance > 200) {
      recommendations.push({
        ...
        description:
          "Grade inconsistency suggests need for structured approach",
        priority: "MEDIUM",
        actionItems: [
    ...
    if (avgRecentGrade > 85 && attendanceRate > 0.9) {
      recommendations.push({
        ...
        description:
          "Excellent performance indicates readiness for advanced challenges",
        priority: "MEDIUM",
        actionItems: [
  ...
  analyzeClassTrends(students: StudentData[]): {
    ...
    const topPerformers = studentAvgGrades
      .slice(0, topCount)
      .map((s) => s.name);
    ...
    const schedule = [
      {
        day: "Monday",
        time: "08:00-09:30",
        focus: "New material introduction",
      },
