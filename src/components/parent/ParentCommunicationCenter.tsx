  ...
  const mockMessages: Message[] = [
    {
      ...
      content:
        "Assalamu'alaikum Bapak/Ibu. Alhamdulillah Ahmad menunjukkan progress yang sangat baik dalam hafalan minggu ini. Dia berhasil menghafal 15 ayat dari Surah Al-Mulk dengan tartil yang baik. Semangat belajarnya juga meningkat dan aktif bertanya saat pembelajaran.",
    ...
    },
    {
      ...
      content:
        "Assalamu'alaikum Ustadz. Jazakallahu khairan atas bimbingan dan perhatiannya kepada Ahmad. Di rumah juga Ahmad semakin rajin muroja'ah dan sering menceritakan pembelajaran di TPQ. Apakah ada yang perlu kami bantu untuk mendukung progress Ahmad?",
      type: "TEXT",
      priority: "LOW",
      isRead: true,
    ...
    },
    {
      ...
      content:
        "Assalamu'alaikum. Kami telah menetapkan goal pengembangan kepemimpinan untuk Ahmad. Dia menunjukkan potensi yang baik untuk memimpin teman-temannya. Mohon dukungan di rumah dengan memberikan tanggung jawab kecil seperti memimpin doa atau mengatur jadwal adik-adiknya.",
      type: "GOAL_UPDATE",
      priority: "MEDIUM",
      isRead: true,
    ...
    },
    {
      ...
      content:
        "Assalamu'alaikum Ustadz. Saya ingin bertanya tentang metode hafalan yang efektif untuk Ahmad di rumah. Apakah ada teknik khusus yang Ustadz rekomendasikan? Dan berapa lama idealnya Ahmad muroja'ah setiap hari?",
      type: "QUESTION",
      priority: "MEDIUM",
      isRead: true,
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span
                      className={`px-2 py-1 text-xs font-medium rounded-full ${getMessageTypeColor(message.type)}`}
                    >
                      {getMessageTypeText(message.type)}
                    </span>
                    <span
                      className={`text-xs font-medium ${getPriorityColor(message.priority)}`}
                    >
                      {message.priority}
                    </span>
                  </div>
                  ...
                  {message.relatedTo && (
                    <div className="mt-2 flex items-center space-x-1 text-xs text-blue-600">
                      {message.relatedTo.type === "GOAL" && (
                        <Target className="h-3 w-3" />
                      )}
                      {message.relatedTo.type === "BEHAVIOR" && (
                        <Heart className="h-3 w-3" />
                      )}
                      {message.relatedTo.type === "ACHIEVEMENT" && (
        ...
        <div className="space-y-3">
          <div className="flex items-center space-x-3">
            <select
              value={messageType}
              onChange={(e) =>
                setMessageType(e.target.value as Message["type"])
              }
              className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent"
            >
