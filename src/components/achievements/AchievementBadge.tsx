...
const AchievementBadge: React.FC<AchievementBadgeProps> = ({
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {/* Badge name */}
      <h3 className={`mt-2 font-semibold text-center ${sizeClasses[size].name}`}>
        {badge.name}
      </h3>
      
      {/* Arabic name */}
      {badge.nameArabic && (
        <p className={`text-gray-600 text-center ${sizeClasses[size].nameArabic}`}>
          {badge.nameArabic}
        </p>
      )}
      ...
      <div className="flex flex-wrap justify-center gap-1 mt-1">
        <span className={`inline-flex items-center rounded-full ${rarityColorClass} ${sizeClasses[size].badges}`}>
          <Star className="w-3 h-3 mr-1" />
          {getRarityText(badge.rarity)}
        </span>
        
        <span className={`inline-flex items-center rounded-full ${categoryColorClass} ${sizeClasses[size].badges}`}>
