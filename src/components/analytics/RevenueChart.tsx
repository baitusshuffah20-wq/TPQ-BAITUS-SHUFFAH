...
export default function RevenueChart({ data, loading }: RevenueChartProps) {
  const [viewType, setViewType] = useState<"revenue" | "transactions">(
    "revenue",
  );
  ...
  const maxValue = Math.max(
    ...data.map((d) => (viewType === "revenue" ? d.revenue : d.transactions)),
  );
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-medium text-gray-900">
                {viewType === "revenue" ? "Revenue" : "Jumlah Transaksi"} per
                Bulan
              </h3>
            ...
            <div className="space-y-3">
              {data.map((item, index) => {
                const value =
                  viewType === "revenue" ? item.revenue : item.transactions;
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {item.growth !== 0 && (
                          <Badge
                            variant={
                              item.growth >= 0 ? "success" : "destructive"
                            }
                            className="text-xs"
                          >
                            {item.growth >= 0 ? "+" : ""}
          ...
          {data.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-500">
                Data akan muncul setelah ada transaksi pembayaran
              </p>
            </div>
          )}
        </div>
