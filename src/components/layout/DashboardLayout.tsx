...
const DashboardLayout = ({ children }: DashboardLayoutProps) => {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      console.log('DashboardLayout: User is unauthenticated, redirecting to login');
      router.push('/login');
    } else {
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          href: `/dashboard/${user?.role?.toLowerCase()}/progress-hafalan`,
          icon: TrendingUp,
          roles: ['ADMIN', 'MUSYRIF', 'WALI']
        },
        {
        ...
        <div className="flex flex-col h-full">
                      ...
                      return (
                        ...
                        >
                          <Icon className={cn(
                            ...
                            active ? "text-teal-600" : "text-gray-400 group-hover:text-gray-500"
                          )} />
                          <span>{item.name}</span>
                          {active && (
          ...
          <div className="border-t border-gray-200 p-4">
            <div className="flex items-center space-x-3 px-2 py-3 rounded-lg hover:bg-gray-50 transition-colors duration-200">
              <div className="relative w-10 h-10 rounded-full overflow-hidden bg-teal-50 ring-2 ring-white">
                {user?.avatar ? (
                  <Image
                    src={user.avatar}
                    alt={user.name || ''}
                  ...
                  <User className="w-full h-full p-2 text-teal-600" />
                )}
              </div>
